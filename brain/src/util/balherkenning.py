    ...
    def getPos(self):
        ...
        for i in range(0,W):
            for j in range(0,H):
                ...
                minwaarde = raw_input("minwaarde: r moet minimaal zo sterk zijn. [0 - 255], std: 180 (anti-zwart)")
                factor = raw_input("factor: r > (g+b)*factor, [0.0-2.0], std: 0.9 (anti-grijs)")
                maxwaarde = raw_input("andere kleuren moeten lager zijn dan dit. [0 - 255], std: 140 (anti-wit)")
                if (color == "red"):
