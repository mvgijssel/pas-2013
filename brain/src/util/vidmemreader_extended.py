...
logging.getLogger('Borg.Brain.Util.VidMemReaderExtended').addHandler(util.nullhandler.NullHandler())

class VidMemReaderExtended():
	...
	"""
        self.logger = logging.getLogger('Borg.Brain.Util.VidMemReaderExtended')
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        vmr = VidMemReaderExtended(["webcam"])
    else:
        sources = sys.argv
        sources.pop(0)
        print sources
        vmr = VidMemReaderExtended(sources)
    while(True):
        image = vmr.get_latest_image()[0]
        cv.ShowImage("vidmemreader_extended", image)
        cv.WaitKey(5)
