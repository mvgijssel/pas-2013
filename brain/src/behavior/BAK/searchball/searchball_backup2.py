...
class Searchball_x(basebehavior.behaviorimplementation.BehaviorImplementation):
    ...
    def implementation_update(self):
        ...
        if (time.time() - self.start_time) > 5:
            if (self.m.n_occurs("combined_red") > 0):
                ...
                if not obs == None and recogtime > self.last_recogtime:
                    ...
                    if blob_max['surface'] > 50:
                        ...
                        #if blob_max['y'] > 75 and blob_max['x'] >10 and blob_max['x'] < 100 and not self.is_looking_horizontal:
                        ...
                        pitch = headstance[1]
                        self.location = self.nao.ball_location(blob_max['x'],blob_max['y'],headstance[0],headstance[1])
            ...
            if (self.idling == 0 and not self.nao.isMoving() and (time.time() - self.start_time) > 13):
                self.temptime = time.time()
            ...
            if self.idling == 2 and (time.time() - self.temptime > 3) and not self.nao.isMoving():
                print "IK GA LEKKER LOPEN LOLOLOL"
                self.nao.look_horizontal()
                self.temptime = time.time()
                if (self.rng < 0.50 ):
            ...
            if self.idling == 3 and (time.time() - self.temptime > 4) and not self.nao.isMoving():
