...
class SoccerDefender_x(basebehavior.behaviorimplementation.BehaviorImplementation):
    ...
    def implementation_init(self):
        ...
        self.nao.say("lahten wuh verdaydiggen")
    ...
    def implementation_update(self):
        ...
        if self.state == "stand" and not self.nao.isMoving() and ((time.time() - self.starttime) > 3) :
            self.nao.set_angles(['HeadPitch'], [0.3267], 0.2, radians=True)
            self.state = "looking down"
            print "Ga squat"
            self.nao.walk(0, 0.00005, 0)
        ...
        if (self.m.n_occurs("combined_red") > 0) and ((time.time() - self.starttime) > 11):
            ...
            if not obs == None and recogtime > self.last_recogtime:
                ...
                if blob_max['surface'] > 150 and ((time.time() - self.looktime) > 2):
                    #self.nao.say("here it comes")
                    self.nao.set_eyes([255,0,0],[255,0,0])
                    headstance =  self.nao.get_yaw_pitch()
                    bal = self.nao.ball_location(blob_max['x'],blob_max['y']+3,headstance[0],headstance[1])                    
                    ...
                    if not self.nao.isMoving() and (time.time() - self.walktime > 2):
                        ...
                        if blob_max['x'] < 70:
                            ...
                            self.nao.walk(-.01, self.movement, -.1)
        ...
        if (self.state == "looking down" or self.state == "left") and ((time.time() - self.looktime) > 4):
            self.looktime = time.time()
            print "looking left"
        ...
        if self.state == "right" and ((time.time() - self.looktime) > 4):
            self.looktime = time.time()
            print "looking right"
            self.nao.start_behavior("rag_look_left2")
            self.state = "left"
